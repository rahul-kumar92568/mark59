 ...
 */
public class DataHunterSeleniumApiTest  implements Serializable {
	 ...
	 * Element waits and  identification will become complex on more difficult applications, but the general elements-within-pages structure should 
	 * generally work.  Although not necessary, the 'Link' elements in this DSL uses the Mark59 SuperPageElement, and this test shows a usage of it when adding policies.  
	 * Used as the basis for the DataHunter PnV script @DataHunterLifecyclePvtScript 
	 */
	@Test
...
// 		add three 'new Business' policies
		...
		for (int i = 1; i <= 3; i++) {
			...
			addPolicyPage = new AddPolicyPage(driver);				
//			addPolicyPage = new AddPolicyPage(driver, true);    // debug mode only    	
...
// 		delete multiple policies (test cleanup)
	...
	/**
	 * Demonstrates capture of the timing of async events using Datahunter.  Note the usage of an alternative DSL to the test above
	 */
	@Test
