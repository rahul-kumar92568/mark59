...
public class DataHunterLifecyclePvtScript  extends SeleniumAbstractJavaSamplerClient {
	...
	private static final Logger LOG = Logger.getLogger(DataHunterLifecyclePvtScript.class);	
	...
	protected Map<String, String> additionalTestParameters() {
		...
		jmeterAdditionalParameters.put(SeleniumDriverFactory.HEADLESS_MODE, String.valueOf(true));
...
//		System.out.println("Thread " + thread + " is running with LOG level " + LOG.getLevel());
		...
		String application = context.getParameter(TestConstants.PARM_NAME_DATAHUNTER_APPLICATION_ID);
		int forceTxnFailPercent = new Integer(context.getParameter(TestConstants.PARM_NAME_FORCE_TXN_FAIL_PERCENT).trim());
		...
		DirectPageGets directPageGets = new DirectPageGets(driver, dataHunterUrl, policySelectionCriteria.getApplication()); 
