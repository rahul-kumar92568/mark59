...
public class ServerMetricsCapture extends AbstractJavaSamplerClient { 
	...
	public static final String CHALLENGE_KEY 		  = "PASSWORD";
	...
	static {
		...
		staticMap.put("________________ server configuration (user/pass blank for localhost,  'HOSTID' or enter value in ALTERNATE SERVER ID to override reported server id) : ________", "");		
		staticMap.put(OPERATING_SYSTEM_KEY,		"UNIX|LINUX|WINDOWS" );
		staticMap.put(SERVER_KEY,				"" );
		staticMap.put(USERNAME_KEY, 			"");
		staticMap.put(CHALLENGE_KEY,			"");
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		ServerMetricsDriverConfig config = new ServerMetricsDriverConfig(
													...
													context.getParameter(CHALLENGE_KEY),
		...
		if (SeverMetricsConstants.WINDOWS.equalsIgnoreCase(config.getOperatingSystem())){
			driver = new DosServerMetricsDriver();
		} else if (	SeverMetricsConstants.UNIX.equalsIgnoreCase(config.getOperatingSystem()) ||
					SeverMetricsConstants.LINUX.equalsIgnoreCase(config.getOperatingSystem()))	{
...
//		additionalTestParametersMap.put(ALTERNATE_SERVER_ID, "CrazyNameForRabbits") ;	
		...
		if ( operatingSystem.indexOf("win") >= 0 ) {
			additionalTestParametersMap.put(OPERATING_SYSTEM_KEY, 	SeverMetricsConstants.WINDOWS);
		} else if ( operatingSystem.indexOf("linux") >= 0 ) {
			additionalTestParametersMap.put(OPERATING_SYSTEM_KEY, 	SeverMetricsConstants.LINUX );
		} else if ( operatingSystem.indexOf("unix") >= 0 ) {
			additionalTestParametersMap.put(OPERATING_SYSTEM_KEY, 	SeverMetricsConstants.UNIX);
		} else {
...
//		additionalTestParametersMap.put(CHALLENGE_KEY, 			"pass");
