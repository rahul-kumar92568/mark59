...
public class DosServerMetricsDriver implements ServerMetricsDriverInterface<ServerMetricsDriverConfig> {
	...
	public Map<String, Long> getCpuMetrics() {
		...
		if (isLocalHost())
			...
			command = MessageFormat.format(WMIC_COMMAND_REMOTE, user, password, server, WMIC_CPU_LOAD_PERCENTAGE);
	...
	private Double getMemoryMetric(boolean isVirtualMemory) {
		...
		if (isLocalHost())
			...
			command = MessageFormat.format(WMIC_COMMAND_REMOTE, user, password, server, memoryType);
